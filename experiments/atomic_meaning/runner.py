...
from product4.authoring.atomic_projection import validate_meaning_plan
...
from product4.workbench.auth import FilesystemUserStore
from product4.workbench.credentials import CredentialVault


MODEL = "gpt-5.4"
ENDPOINT = "https://api.openai.com/v1/chat/completions"
EXPECTED_PROJECT = os.environ.get("PRODUCT4_EXPERIMENT_PROJECT_ID", "").strip()
EXPECTED_KEY_SUFFIX = os.environ.get("PRODUCT4_EXPERIMENT_KEY_SUFFIX", "").strip()
DATA_ROOT_VALUE = os.environ.get("PRODUCT4_EXPERIMENT_DATA_ROOT", "").strip()
DATA_ROOT = Path(DATA_ROOT_VALUE or ".")
OUTPUT_ROOT = Path(os.environ.get("PRODUCT4_EXPERIMENT_OUTPUT_ROOT", "/tmp"))
ARTIFACT = OUTPUT_ROOT / "autoglific-atomic-long-result.json"
CHECKPOINT = OUTPUT_ROOT / "autoglific-atomic-long-checkpoint.json"


PROSE = (
    'When a user sends CHECKUP on WhatsApp, start the community clinic registration flow. '
    'Send "Welcome to community clinic registration." '
    'Ask "Would you like to register for a clinic visit?" and let them select exactly one of Yes or No. '
    'If they select No, send "No problem. You can return whenever you need us." and end with the outcome declined_registration. '
    'If they select Yes, ask "What is your full name?" and capture a text response. '
    'Then ask "What is your age?" and capture a number response. '
    'Then ask "What email should receive the registration confirmation?" and capture an email response. '
    'Next ask "Which clinic location do you prefer?" and let them select exactly one of North Clinic or South Clinic. '
    'After either location, ask "Which time do you prefer?" and let them select exactly one of Morning or Afternoon. '
    'Then ask "Do you need transport assistance?" and let them select exactly one of Yes or No. '
    'If they select Yes for transport, ask "What pickup address should we use?" and capture a text response. '
    'After either transport choice, ask "How urgent are your symptoms?" and let them select exactly one of Mild, Moderate, or Severe. '
    'If they select Severe, send "If this is an emergency, contact local emergency services now." and then ask "What phone number can the clinic call?" and capture a phone response. '
    'After every urgency path, ask "Is there anything else the clinic should know?" and capture a text response. '
    'Then send "Thank you. Your clinic registration has been recorded." and end with the outcome completed_registration.'
)
...
def credentials() -> tuple[str, str]:
    if not EXPECTED_PROJECT or not EXPECTED_KEY_SUFFIX or not DATA_ROOT_VALUE:
        raise RuntimeError("P4_EXPERIMENT_CREDENTIAL_EXPECTATION_MISSING")
    store = FilesystemUserStore(DATA_ROOT)
    user_files = list((DATA_ROOT / "auth" / "users").glob("*.json"))
    if len(user_files) != 1:
        raise RuntimeError("P4_EXPERIMENT_CREDENTIAL_OWNER_AMBIGUOUS")
    user_id = json.loads(user_files[0].read_text())["user_id"]
    key = (DATA_ROOT / "credential-encryption.key").read_bytes().strip()
    values = CredentialVault(store, encryption_key=key).values(user_id)
    api_key = values.get("openai_api_key", "")
    project = values.get("openai_project_id", "")
    if not api_key.endswith(EXPECTED_KEY_SUFFIX) or project != EXPECTED_PROJECT:
        raise RuntimeError("P4_EXPERIMENT_CREDENTIAL_PAIR_MISMATCH")
...
def provider_call(api_key: str, project: str, *, name: str, system: str, user: str, schema: dict[str, Any]) -> tuple[dict[str, Any], dict[str, Any]]:
    ...
    try:
        ...
        raise RuntimeError(f"P4_EXPERIMENT_PROVIDER_HTTP_{exc.code}:{request_id or '-'}") from exc
    elapsed = round(time.monotonic() - started, 2)
    choice = envelope["choices"][0]["message"]
    if choice.get("refusal"):
        raise RuntimeError("P4_EXPERIMENT_PROVIDER_REFUSAL")
...
def exact_graph_errors(graph: dict[str, Any]) -> list[str]:
    errors: list[str] = []
    nodes = graph["nodes"]
    ...
    errors.extend(f"ungrounded_span:{span}" for span in spans if span not in PROSE)
    return errors
...
def topological_order(graph: dict[str, Any]) -> list[str]:
    ...
    if len(result) != len(ids):
        raise RuntimeError("P4_EXPERIMENT_GRAPH_CYCLE")
    return result
...
def branch_priority_topological_order(
    ...
    while ready:
        ...
        if not current_ranks:
            raise RuntimeError(f"P4_EXPERIMENT_NODE_UNREACHABLE:{current}")
        ...
        for edge in outgoing[current]:
            ...
            if option_ranks:
                ...
                if not selected or any(label not in option_ranks for label in selected):
                    raise RuntimeError(f"P4_EXPERIMENT_CHOICE_EDGE_INVALID:{current}:{condition}")
                ranks[target].update(
...
def bind_configurations(
    ...
    if unknown_answers:
        raise ValueError("P4_EXPERIMENT_UNKNOWN_CLARIFICATION_ANSWERS:" + ",".join(unknown_answers))
...
def phrase_configuration_questions(
    ...
    if actual != expected or len(actual) != len(set(actual)):
        raise RuntimeError("P4_EXPERIMENT_CONFIGURATION_QUESTION_COVERAGE_INVALID")
...
def compile_plan(
    ...
    if configuration["gaps"]:
        raise RuntimeError("P4_EXPERIMENT_CONFIGURATION_GAPS_UNANSWERED")
    ...
    for node_id in order:
        ...
        if not paths:
            raise RuntimeError(f"P4_EXPERIMENT_NODE_UNREACHABLE:{node_id}")
        if capability == "fixed_choice":
            ...
            for edge in outgoing[node_id]:
                ...
                if not selected or any(label not in option_map for label in selected):
                    raise RuntimeError(f"P4_EXPERIMENT_CHOICE_EDGE_INVALID:{node_id}:{condition}")
            ...
            for edge in outgoing[node_id]:
                if edge.get("condition") is not None:
                    raise RuntimeError(f"P4_EXPERIMENT_NON_CHOICE_CONDITION:{node_id}")
...
def main() -> None:
    ...
    CHECKPOINT.write_text(json.dumps({
        "prose": PROSE,
        "credentials": {"project": project, "key_suffix": EXPECTED_KEY_SUFFIX, "model": MODEL, "reasoning": "high"},
        "calls": {"graph": graph_call},
        "graph": graph,
    }, ensure_ascii=False, indent=2))
    graph_errors = exact_graph_errors(graph)
    if graph_errors:
        raise RuntimeError("P4_EXPERIMENT_GRAPH_INVALID:" + ",".join(graph_errors))
    binder_user = json.dumps({"registry": registry_payload(), "graph": graph}, ensure_ascii=False)
    ...
    if unsupported:
        raise RuntimeError("P4_EXPERIMENT_UNSUPPORTED_NODES:" + ",".join(row["semantic_node_id"] for row in unsupported))
    ...
    if configuration["gaps"]:
        ...
        checkpoint = {
            ...
            "credentials": {
                "project": project,
                "key_suffix": EXPECTED_KEY_SUFFIX,
                "model": MODEL,
                "reasoning": "high",
            },
            "calls": {
        ...
        ARTIFACT.write_text(json.dumps(checkpoint, ensure_ascii=False, indent=2))
        print(json.dumps({
            "status": "awaiting_configuration",
            "questions": len(questions),
            "generated_variable_names": {
                node_id: values["save_as"]
                for node_id, values in configuration["configs"].items()
                if "save_as" in values
            },
        }, ensure_ascii=False))
        return
    plan, compilation = compile_plan(graph, binding_map)
    ...
    result = {
        ...
        "summary": {
            ...
            "missing_fact_fields": {key: value for key, value in compilation["fact_gaps"].items() if value},
            "active_configuration_findings": len(active_findings),
            "executable_errors": executable_errors,
        },
    }
    ARTIFACT.write_text(json.dumps(result, ensure_ascii=False, indent=2))
    print(json.dumps(result["summary"], ensure_ascii=False))
    print(json.dumps({"calls": result["calls"], "artifact": str(ARTIFACT)}, ensure_ascii=False))
